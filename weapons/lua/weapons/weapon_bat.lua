...
if Tr.Hit then
		...
		local Size = math.Clamp(1 - ((Tr.HitPos - self.Owner:GetShootPos()):Length() / 90) ^ 2, .1, .3)
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		self.Owner.stamina = math.max(self.Owner.stamina - 8,0)
	end
	...
	if true then
		if SERVER and tr.HitWorld then
			self.Owner:EmitSound(  "physics/wood/wood_plank_impact_hard"..math.random(1,5)..".wav",60  )
		end
		...
		if IsValid( tr.Entity ) and SERVER then
			...
			if tr.Entity:IsNPC() or tr.Entity:IsPlayer() then
				self.Owner:EmitSound( "Flesh.ImpactHard",60 )
			else

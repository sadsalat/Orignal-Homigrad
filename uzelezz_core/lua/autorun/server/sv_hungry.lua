function hungry()
hungry_NextThink=0
hook.Add("Think","sayhungry",function()
	for i, v in ipairs( player.GetAll() ) do
		...
		v.hungry_NextThink=CurTime()+0.5
		if v.HasLeft==nil then
			if not v:Alive() then return end
			v:SetNWInt("hungryregen",math.Clamp(v:GetNWInt("hungryregen")-0.25, -0.05, 10))
			if v:GetNWInt("hungryregen")==nil then v:SetNWInt("hungryregen",math.Clamp(v:GetNWInt("hungryregen")-0.25, -0.01, 10)) end
...
hook.Add("PlayerDeath", "deathhungry", function(v)
v.hungry = 89
v:SetNWInt("hungryregen",0)
end)
hook.Add("PlayerSpawn", "spawnhungry", function(v)
v.hungry = 89

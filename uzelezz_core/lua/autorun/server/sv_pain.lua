...
function Pain()
	...
	hook.Add("EntityTakeDamage","PlayerPainGrowth",function(ply,dmginfo)
		if ply:IsPlayer() or IsValid(RagdollOwner(ply)) then
			...
			if (ply.pain>(250*(ply.Blood/5000))+(ply:GetNWInt("SharpenAMT")*5) or ply.Blood<3000) and !ply.Otrub  then
				ply.gotuncon=true
			end
		end
	end)
	...
	hook.Add("Think","PainLosingValue",function()
		for i, ply in pairs(player.GetAll()) do
			ply.pain=ply.pain or 0
			ply.painlosing=ply.painlosing or 1
			if ply:GetNWInt("painlosing")==nil then ply:SetNWInt("painlosing",1) end
	...
	hook.Add("PlayerDeath","RefreshPain",function(ply)
		ply.pain = 0
		ply.painlosing = 0
...
function GetUnconscious(ply)
	...
	ply:ConCommand( "soundfade 90 99999" )
	else
		ply:ConCommand( "soundfade 0 1" )
	end
	if !ply.fake then
	...
	if IsValid(rag) and RagdollOwner(rag).Otrub then rag:SetEyeTarget(Vector(0,0,0)) end
end

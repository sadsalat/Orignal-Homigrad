...
SWEP.Author = "Atun?"
SWEP.Purpose = "Долговечная консерва, вкус так себе но сытно."
SWEP.Slot = 2
SWEP.SlotPos = 3
SWEP.Spawnable = true
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		self:GetOwner():SetNWInt("hungryregen", self:GetOwner():GetNWInt("hungryregen") + 2)
		self:Remove()
...
function SWEP:SecondaryAttack()
	...
	if SERVER then
		...
		if IsValid(WhomILookinAt) and WhomILookinAt:IsPlayer() then
			self:Remove()
			self:GetOwner():SelectWeapon("weapon_hands")
			WhomILookinAt:SetNWInt("hungryregen", self:GetOwner():GetNWInt("hungryregen") + 2)
			sound.Play(healsound, self:GetPos(), 75, 100, 0.5)
		end

		if WhomILookinAt:IsRagdoll() then
			if not IsValid(RagdollOwner(WhomILookinAt)) then return nil end
			RagdollOwner(WhomILookinAt):SetNWInt("hungryregen", self:GetOwner():GetNWInt("hungryregen") + 2)
			self:Remove()
			sound.Play(healsound, self:GetPos(), 75, 100, 0.5)
			self:GetOwner():SelectWeapon("weapon_hands")
		end
